	...
	contours:
		0:
			...
			nodes:
				...
				1:
					x: Math.max( contours[0].nodes[2].expandedTo[1].x - thickness / 3, contours[0].nodes[2].x + ( contours[0].nodes[0].x - contours[0].nodes[2].x ) * ( 45 / 110 ) )
					y: - overshoot / 2
					dirOut: 180 + 'deg'
					type: 'smooth'
					expand: Object({
		...
		2:
			...
			nodes:
				...
				1:
					...
					y: contours[0].nodes[5].expandedTo[1].y - 60 * serifBall
					dirOut: 0 + 'deg'
					type: 'smooth'
				2:
